...
let Demo = React.createClass({
  ...
  getItems () {
    ...
    switch (this.props.route.menus[this.state.currentMenu].items) {
      case 1:
        items = [
  ...
  render () {
    const buttons = Object.keys(this.props.route.menus).map((menu) => {
      return (
        <a key={menu}
          className={classNames({'current-demo': menu === this.state.currentMenu})}
          onClick={this.changeMenu.bind(this, menu)}>
          {this.props.route.menus[menu].buttonText}
        </a>
      )
    })
